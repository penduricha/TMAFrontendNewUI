      ...
      <div class="flex-item flex-item-2">
        <div class="flex-item flex-item-2 flex-item-2-1" style="display: flex;">
          ...
          <div style="margin-left: 20px">
            <p style="color: white; font-size: 15px; margin-bottom: 0">
              {{Math.round(rateRegister*100)/100}}% {{compareStatementRegister}}
            </p>
            <p style="color: white; font-size: 20px; margin-top: 0; margin-bottom: 0">
              {{numberRegisterToday}} Register(s) today.
            </p>
            <p style="color: white; font-size: 15px; margin-bottom: 0">
              Today's visits
            </p>
        ...
        <div class="flex-item flex-item-2 flex-item-2-2" style="display: flex;">
          ...
          <div style="margin-left: 20px">
            ...
            <p style="color: white; font-size: 25px; margin-top: 0; margin-bottom: 0">
              {{numberLoginToday}} Login(s) today
            </p>
            <p style="color: white; font-size: 12px; margin-bottom: 0">
              Today's visits
            </p>
        ...
        <div class="flex-item flex-item-2 flex-item-2-3" style="display: flex;">
          ...
          <div style="margin-left: 20px">
            <p style="color: white; font-size: 12px; margin-bottom: 0">
              {{Math.round(rateButtonSearch*100)/100}}% {{compareButtonSearch}}
            </p>
            <p style="color: white; font-size: 25px; margin-top: 0; margin-bottom: 0">
              {{numberButtonSearchToday}} Search(es) today
            </p>
            <p style="color: white; font-size: 12px; margin-bottom: 0">
              Today's visits
            </p>
...
export default {
  ...
  created(){
    ...
    this.countButtonSearch_SevenDay();
    this.getNumberButtonSearch();
  },
  mounted() {
    //get
    this.getRateRegisterToday();
    this.getRateLoginToday();
    this.getRateButtonSearch();
  },
  beforeMount() {
  ...
  data(){
    return{
      ...
      //button search
      numberButtonSearchToday: null,
      rateButtonSearch: null,
      compareButtonSearch: null,
  ...
  methods:{
    ...
    //Button Search
    async countButtonSearchToday(){
      let number=null;
      await ButtonSearch.countButtonSearchToday().then(response => {
        number= response.data;
        console.log('Number button search today: ',number);
      }).catch(error => {
        console.error(error);
      });
      return number;
    ...
    async countButtonSearch_SevenDay(){
      let number=null;
      await ButtonSearch.countButtonSearch_SevenDay().then(response => {
        number= response.data;
        console.log('Number button search 7 days: ',number);
      }).catch(error => {
        console.error(error);
      });
      return number;
    ...
    async getNumberButtonSearch(){
      this.numberButtonSearchToday=await this.countButtonSearchToday();
    },

    async getRateButtonSearch(){
      let buttonSearchSevenDay=await this.countButtonSearch_SevenDay();

      if(buttonSearchSevenDay===0 || this.numberButtonSearchToday===0){
        this.rateButtonSearch=0;
        this.compareButtonSearch='There are no search in today or 7 days lasted.';
      }else{
        if(buttonSearchSevenDay===this.numberButtonSearchToday){
          this.rateButtonSearch=0;
          this.compareButtonSearch='There are no search in today or 7 days lasted.';
        }
        if(buttonSearchSevenDay > this.numberButtonSearchToday){
          this.rateButtonSearch=100-(this.numberButtonSearchToday/buttonSearchSevenDay)*100;
          this.compareButtonSearch='lower than 7 days lasted';
        }
        if(buttonSearchSevenDay < this.numberButtonSearchToday){
          this.rateButtonSearch=100-(buttonSearchSevenDay/this.numberButtonSearchToday)*100;
          this.compareButtonSearch='higher than 7 days lasted';
        }
      }
    },
  },
